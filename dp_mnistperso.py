...
tf.flags.DEFINE_float("lr_saturate_epochs", 10,
...
tf.flags.DEFINE_string("training_data_path",
                       "/Users/laurent/Desktop/DP/data/mnist_train.tfrecord",
                       "Location of the training data.")
tf.flags.DEFINE_string("eval_data_path",
                       "/Users/laurent/Desktop/DP/data/mnist_test.tfrecord",
...
tf.flags.DEFINE_string("save_path", "/Users/laurent/Desktop/DP/models",
...
# def predict(images_input, network_parameters,
...
def predict(sess,images_input):
    ...
    predictions= logits.eval(session = sess,feed_dict={images:images_input})
...
def Train(sess,train_images,train_labels, mnist_test_file, network_parameters, num_steps,
          save_path,training_params, eval_steps=0):
    ...
    for step in xrange(num_steps):
        ...
        lot = np.random.choice(np.arange(NUM_TRAINING_IMAGES),lot_size,replace=False)
        #print lot.shape
        ...
        for i in xrange(FLAGS.batches_per_lot):
            _ = sess.run([gd_op], feed_dict={lr: curr_lr, eps: curr_eps, delta: FLAGS.delta,
                        images: images_lot[i*FLAGS.batch_size:(i+1)*FLAGS.batch_size,:],
                        labels: labels_lot[i*FLAGS.batch_size:(i+1)*FLAGS.batch_size]})
        ...
        if (eval_steps > 0 and (step + 1) % eval_steps == 0) or should_terminate:
            if with_privacy:
                ...
                while index_eps_to_save<len(spent_eps_deltas) and spent_eps_deltas[index_eps_to_save][1]<FLAGS.delta:
                    saver.save(sess, save_path=save_path +
                    "/eps"+str(spent_eps_deltas[index_eps_to_save][0])+
                    "_delta"+'%.2g' % spent_eps_deltas[index_eps_to_save][1]+
                    "_pcasigma" +str(FLAGS.pca_sigma)+
                    "_sigma" +str(FLAGS.sigma)+
                    "/ckpt")
                    index_eps_to_save+=1
            else:
